...
function deljob(){
	...
	if($_POST['param'] > 0){
		$chenge->delete("book","book_id=" . $_POST['param']);
	}
}
...
<a href="book_detail.php?pid={$rows[$key]['book_id']}{$urlparam}">{$rows[$key]['book_title']}</a>
</td>
<td width="15%" class="center{$nobottom}">
<input type="button" value="削除確認" onClick="javascript:del_func_form({$rows[$key]['book_id']},'{$javamsg}')" />
</td>
</tr>
END_BLOCK;
		$retstr .= $str;
		...
		$retstr =<<<END_BLOCK
<tr><td colspan="3" class="nobottom">書籍が見つかりません</td></tr>
END_BLOCK;
	}
	echo $retstr;
}
